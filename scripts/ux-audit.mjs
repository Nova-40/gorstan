...
const AUDIT_COMMANDS = {
  ...
  'images:scan': {
    ...
    fallback: 'Get-ChildItem public\\images -Recurse -Include *.png,*.jpg,*.jpeg,*.gif,*.svg | Select-Object Name,Length'
  },
  'ux:a11y': {
...
class UXAuditor {
  ...
  async runCommand(key, config) {
    ...
    try {
      ...
      console.log(`   Error: ${error.message.slice(0, 200)}${error.message.length > 200 ? '...' : ''}`);
