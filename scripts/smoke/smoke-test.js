...
class GorstnSmokeTest {
  ...
  async testInitialRender() {
    ...
    const gameElements = await this.page.$$eval(
      '[class*="game"], [id*="game"], [class*="terminal"], [class*="console"]',
  ...
  async testConsoleCommands() {
    ...
    const commandInput = await this.page.$(
  ...
  async testInventoryAccess() {
    ...
    const inventoryButton = await this.page.$(
      'button[title*="Inventory"], button:has-text("Inventory"), button[title*="Backpack"]',
    );
    ...
    try {
      ...
      const inventoryOpen = await this.page.$(
        '[class*="inventory"], [role="dialog"]:has-text("Inventory")',
      );
      if (inventoryOpen) {
