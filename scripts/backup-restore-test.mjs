...
async function runBuildTest(testDir) {
  ...
  try {
    ...
    try {
      execSync('npm ci', { stdio: 'inherit', cwd: projectDir });
    ...
    try {
      execSync('npm run typecheck', { stdio: 'inherit', cwd: projectDir });
      success('TypeScript check passed');
    } catch (err) {
      warn('TypeScript check failed, but continuing...');
    }
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      if (!buildValid) {
        warn('Build test failed, but continuing (this may be due to environment differences)');
      }
    } catch (err) {
      warn(`Build test failed: ${err.message}`);
      warn('This may be due to environment differences, but restore succeeded');
    }
