...
export class DeviceProfiler {
  ...
  private estimateMemory(): number {
    ...
    if (screenPixels > 2073600 && cores >= 8) {
      return 16;
    } // 4K+ display, many cores
    if (screenPixels > 1440000 && cores >= 4) {
      return 8;
    } // 1080p+ display, quad core
    if (screenPixels > 921600 && cores >= 2) {
      return 4;
    } // 720p+ display, dual core
  ...
  private calculatePerformanceTier(capabilities: DeviceCapabilities): 'low' | 'medium' | 'high' {
    ...
    if (capabilities.cpuCores >= 8) {
      ...
      score += 2;
    } else if (capabilities.cpuCores >= 2) {
      score += 1;
    }

    // Memory score (0-3 points)
    if (capabilities.memoryGB >= 16) {
      score += 3;
    ...
    // WebGL support
    if (capabilities.supportsWebGL && capabilities.maxTextureSize >= 8192) {
      score += 1;
    }
