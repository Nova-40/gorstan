...
class ShadowEncounterService {
  ...
  private getEligibleEntities(
    routeId: RouteId,
    routeConfig: RouteSpecificShadowConfig,
  ): ShadowEntity[] {
    return Object.values(SHADOW_ENTITIES).filter((entity) => {
      ...
      if (
        entity.spawnConditions.routeSpecific &&
      ...
      if (
        entity.spawnConditions.quantumLevel &&
  ...
  private calculateEncounterDifficulty(entity: ShadowEntity): number {
    let difficulty = Math.floor(
      (entity.stats.health + entity.stats.aggression + entity.stats.awareness) / 30,
    );
  ...
  public interactWithShadow(
    ...
    const encounter = Array.from(this.shadowState.encounterState.activeEncounters.values()).find(
      (enc) => enc.entityId === entityId,
    );
    if (!encounter) {
      return null;
    }
    ...
    if (interaction.result.success && ['banish', 'retreat'].includes(interactionType)) {
  ...
  private processInteraction(
    ...
    // Update stress
    this.shadowState.encounterState.playerStress = Math.max(
      0,
  ...
  private calculateArtifactEffectiveness(entity: ShadowEntity, artifactIds: string[]): number {
    ...
    for (const artifactId of artifactIds) {
      const effect = Object.values(QUANTUM_SHADOW_EFFECTS).find(
        (effect) => effect.artifactId === artifactId,
      );
  ...
  private calculateBanishPower(artifactIds: string[]): number {
    return this.calculateArtifactEffectiveness(
      { weaknesses: ['quantum', 'artifacts'] } as ShadowEntity,
      artifactIds,
    );
  }
  ...
  private getEntityMoodDescription(entity: ShadowEntity): string {
    ...
    if (manifestation < 60) {
      return 'partially solid';
    }
    if (manifestation < 90) {
      return 'clearly defined';
    }
  ...
  private updateAdaptiveDifficulty(): void {
    ...
    const successRate =
      recentEncounters.filter((enc) => enc.outcome === 'success').length / recentEncounters.length;
    ...
    this.shadowState.adaptiveDifficulty.adjustmentFactor = Math.max(
      0.5,
  ...
  private increaseStress(amount: number): void {
    this.shadowState.encounterState.playerStress = Math.min(
      100,
