...
export interface DynamicContent {
  type: 'room_description' | 'ambient_event' | 'contextual_story' | 'adaptive_dialogue';
  content: string;
  triggers: string[];
...
export class UnifiedAIIntelligence {
   ...
   */
  async getUnifiedGuidance(context: UnifiedAIContext): Promise<AIGuidanceResponse | null> {
    if (!this.aiEnabled || Date.now() - this.lastGuidanceTime < this.guidanceCooldown) {
      return null;
    }
   ...
   */
  private async getAylaGuidance(context: UnifiedAIContext): Promise<AIGuidanceResponse | null> {
...
Ayla's guidance:`;
   ...
   */
  private recordCrossSystemLearning(context: UnifiedAIContext, response: AIGuidanceResponse): void {
