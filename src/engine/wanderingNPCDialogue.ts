...
export const aylaDialogue: DialogueTree = {
  ...
  about: [
    ...
    {
      text: 'Think of me as a friendly face in the machine, {playerName} - literally.',
      chance: 0.1,
    },
  ],
...
export const dominicDialogue: DialogueTree = {
  ...
  breaking_fourth_wall: [
    ...
    {
      text: "*knowing look* I exist in every timeline where you didn't take me. Think about that.",
      chance: 0.05,
    },
  ],
...
export function getWanderingNPCResponse(
  ...
  switch (npcId) {
    case 'mr_wendell':
      dialogue = mrWendellDialogue;
      break;
    case 'ayla':
      dialogue = aylaDialogue;
      break;
    case 'al_escape_artist':
      dialogue = alDialogue;
      break;
    case 'dominic_wandering':
      dialogue = dominicDialogue;
      break;
    case 'morthos':
      dialogue = morthosDialogue;
      break;
    case 'polly':
      dialogue = pollyDialogue;
      break;
    case 'albie':
      dialogue = albieDialogue;
      break;
    default:
...
export function getNPCInteractionLine(npc1: string, npc2: string): string | null {
  const interactions: Record<string, Record<string, string[]>> = {
    ayla: {
      polly: [
        "*Ayla's presence causes Polly to step back nervously*",
        "*gentle but firm* You don't belong here right now.",
      ],
    ...
    morthos: {
      ...
      dominic_wandering: [
        "*Morthos stares at the fish* Even you can't escape the darkness forever.",
      ],
    },
  };
