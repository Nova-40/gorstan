...
export const ConditionHelpers = {
  ...
  hasNPC: (npcsInRoom: NPC[], npcName: string): boolean => {
    try {
      return npcsInRoom.some((npc) => {
        return npc.name.toLowerCase().trim() === npcName.toLowerCase().trim();
      });
    } catch (error) {
  ...
  getReputation: (playerState: PlayerState | undefined): number => {
    try {
      ...
      if (playerState.reputation) {
        reputationValues = Object.values(playerState.reputation);
        if (reputationValues.length > 0) {
          return Math.max(...reputationValues);
        }
      }
...
export function checkAlbieIntervention(
  ...
  try {
    ...
    const interventionResult = checkNPCInterventions(context);
    return interventionResult.occurred;
  } catch (error) {
  ...
  try {
    ...
    return interventionRules
      .map((rule) => {
        ...
        if (!isNPCPresent) {
          reason = 'NPC not present';
        } else if (suppressedNPCs.length === 0) {
          reason = 'No targets to suppress';
        } else if (!canTrigger) {
          reason = 'Cooldown/limit reached';
        } else if (!conditionsMet) {
          reason = `Conditions not met${conditionError}`;
        }
