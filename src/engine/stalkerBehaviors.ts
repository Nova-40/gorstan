...
export function handleWendellStalking(state: LocalGameState, dispatch: Dispatch<GameAction>): void {
  ...
  if (state.currentRoomId !== stalkerState.lastSeenRoom) {
    ...
    setTimeout(() => {
      if (wendell.currentRoom !== stalkerState.lastSeenRoom) {
        ...
        console.log(
          `[STALKER] Mr. Wendell moved from ${previousRoom} to ${wendell.currentRoom} (following player trail)`,
        );

        // Chance for ominous message when Wendell moves
        if (
          Math.random() < 0.3 &&
          currentTime - stalkerState.lastMessageTime > stalkerState.cooldown
        ) {
  ...
  if (
    Math.random() < 0.05 &&
    currentTime - stalkerState.lastMessageTime > stalkerState.cooldown * 2
  ) {
...
export function handlePollyStalking(state: LocalGameState, dispatch: Dispatch<GameAction>): void {
  ...
  if (!state.flags?.dominicIsDead && !state.flags?.dominicTakenAfterWarning) {
    return;
  }
  ...
  if (state.currentRoomId !== stalkerState.lastSeenRoom) {
    ...
    setTimeout(() => {
      ...
      if (polly.currentRoom !== targetRoom) {
        ...
        console.log(
          `[STALKER] Polly moved from ${previousRoom} to ${polly.currentRoom} (vengeful stalking)`,
        );

        // Message when Polly appears
        if (
          shouldAppearInSameRoom &&
          currentTime - stalkerState.lastMessageTime > stalkerState.cooldown
        ) {
...
function checkWendellStalkingTriggers(state: LocalGameState): boolean {
  ...
  if (cursedItems.some((item) => inventory.includes(item))) {
    return true;
  }

  // Wendell stalks if player has killed or harmed NPCs
  if (flags.dominicIsDead || flags.npcKilled) {
    return true;
  }
...
function getWendellStalkerMessage(intensity: number): string {
  ...
  const picked = categoryMessages[Math.floor(Math.random() * categoryMessages.length)];
  return picked ?? categoryMessages[0] ?? '';
}
...
function handlePollyEscalation(
  ...
  dispatch({
    type: 'SET_FLAG',
    payload: { flag: 'pollyIntensity', value: stalkerState.intensity },
  });
}
