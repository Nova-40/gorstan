...
function processItemEffect(effect: ItemEffect, result: ItemUseResult, playerState: any): void {
  switch (effect.type) {
    case 'message':
    ...
    case 'trait':
      if (effect.value && result.traitChanges) {
        if (!result.traitChanges.add) {
          result.traitChanges.add = [];
        }
    ...
    case 'transform':
      if (effect.target) {
        ...
        if (!result.inventoryChanges.remove) {
          result.inventoryChanges.remove = [];
        }
        if (!result.inventoryChanges.add) {
          result.inventoryChanges.add = [];
        }
...
export function transformItem(
  ...
  if (!sourceItem || !sourceItem.transformInto) {
    return null;
  }
