...
export function checkForTrap(
  roomName: string,
  triggerType: 'enter' | 'exit' | 'search' | 'take' | 'use' = 'enter',
): string | null {
  try {
...
function getWeightedRandomType(
  weights: Record<string, number>,
): 'environmental' | 'magical' | 'mechanical' | 'cursed' {
  const rand = Math.random();
  let cumulative = 0;
...
function checkTrapRequirements(
  requirements: NonNullable<TrapData['requirements']>,
  playerState: PlayerState,
): boolean {
  try {
...
export function getTrapStatistics(): TrapStatistics {
  try {
    ...
    const trapsByType = allTraps.reduce(
      (acc, trap) => {
    ...
    const trapsBySeverity = allTraps.reduce(
      (acc, trap) => {
    ...
    const trapsByRoom = allTraps.reduce(
      (acc, trap) => {
    ...
    const active = allTraps.filter(
      (trap) => !(trap.triggered ?? false) || (trap.repeatTrigger ?? false),
    ).length;
...
export function batchTrapOperations(
  operations: Array<{
...
export function loadTrapState(): boolean {
  try {
    ...
    console.log(
      `[TrapController] Loaded ${Object.keys(activeTraps).length} traps from saved state`,
    );
    return true;
  } catch (error) {
