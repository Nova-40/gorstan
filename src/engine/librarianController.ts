...
function handleGreasyNapkin(gameState: LocalGameState, dispatch: React.Dispatch<GameAction>): void {
  dispatch({
    type: 'ADD_MESSAGE',
    payload: {
      id: Date.now().toString(),
...
export function handleGuardQuestion(
  ...
  // Variable declaration
  const isCorrectQuestion = correctPatterns.some(
    (pattern) =>
...
export function handleDoorChoice(
  ...
  // Variable declaration
  const isCorrect =
    librarianState.puzzleStage === 'question_asked' && chosenDoor === librarianState.correctDoor;
