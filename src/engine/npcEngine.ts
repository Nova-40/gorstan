...
export function npcReact(npcId: string, playerInput: string, gameState?: LocalGameState | null) {
  ...
  const response =
    typeof matched.response === 'function' ? matched.response(state, memory) : matched.response;
  npcSpeak(npcId, response, matched.tone || 'neutral');
  if (matched.onRespond) {
    matched.onRespond(state, memory);
  }
}
