...
export function wanderNPC(npcId: string, state: LocalGameState) {
  ...
  if (!npc || !npc.canWander || npc.questOnly) {
    return;
  }

  const now = Date.now();
  if (npc.lastMoved && now - npc.lastMoved < 15000) {
    return;
  } // 15 second cooldown
...
export function initializeWanderingNPCs(state: LocalGameState, dispatch: Dispatch<GameAction>) {
  for (const [npcId, npc] of npcRegistry.entries()) {
