...
export const wanderingNPCs: WanderingNPC[] = [
  {
    ...
    name: 'Mr. Wendell',
    entryLine:
      'A chill settles over the room as Mr. Wendell materializes from the shadows, standing perfectly still.',
    ...
    spawnConditions: [{ type: 'custom', key: 'wendell_logic' }],
    conflictRules: [{ type: 'displaces', condition: 'always' }],
  ...
  },
  {
    ...
    exitLine:
      'The Librarian retreats into the depths of knowledge, his footsteps echoing among ancient tomes.',
    personality: {
    ...
    spawnConditions: [{ type: 'custom', key: 'librarian_logic' }],
    conflictRules: [{ type: 'displaces', condition: 'in_library' }],
...

  {
    ...
    name: 'Dominic',
    entryLine:
      'A goldfish bowl materializes on a nearby surface, Dominic swimming in contemplative circles.',
    ...
    conflictRules: [{ type: 'cannot_coexist', targetNPC: 'polly' }],
...

  {
    ...
    name: 'Morthos',
    entryLine:
      'Shadows gather in the corners as Morthos emerges, his presence both threatening and oddly reassuring.',
...

  {
    ...
    name: 'Polly',
    entryLine:
      "Polly steps out from behind something that wasn't there a moment ago, watching you intently.",
...

  {
    ...
    name: 'Albie',
    entryLine:
      'Albie strolls in with the confidence of someone who owns the place, nodding professionally.',
...
function canNPCSpawnInRoom(
  ...
  if (
    !npc.spawnConditions.every((condition) =>
...
function evaluateSpawnCondition(
  ...
  switch (condition.type) {
    case 'flag':
      if (!condition.key || !playerState.flags) {
        return false;
      }
      // Variable declaration
    ...
    case 'inventory':
      if (!condition.key || !playerState.inventory) {
        return false;
      }
      // Variable declaration
...
function getRoomZone(roomId: string, room: Room): string {
  ...
  if (description.includes('stable')) {
    return 'stable';
  }
  if (roomId.includes('glitch')) {
    return 'surreal';
  }
  if (roomId.includes('lattice')) {
    return 'surreal';
  }
...
export function shouldSpawnNPC(npc: WanderingNPC, state: PlayerState): boolean {
  if ((npc as any).questOnly && state.flags && !(state.flags as any)[(npc as any).requiredFlag]) {
    return false;
  }
  return true;
}
