...
// Variable declaration
    const puzzleState = (gameState as any).puzzleState || {};
// Variable declaration
    const currentState = puzzleState[puzzleId] || { attempts: 0, solved: false, hintsUsed: [] };

    
    if (puzzle.maxAttempts && currentState.attempts >= puzzle.maxAttempts) {
      return {
        success: false,
        feedback: 'You have reached the maximum number of attempts for this puzzle.'
      };
    }
...
// Variable declaration
  ...
  private validatePatternPuzzle(puzzle: PuzzleData, solution: any): PuzzleResult {
...
// Variable declaration
  ...
  private validateNavigationPuzzle(puzzle: PuzzleData, solution: any): PuzzleResult {
...
// Variable declaration
  ...
  private validateSequencePuzzle(puzzle: PuzzleData, solution: any): PuzzleResult {
...
// Variable declaration
  ...
  private registerCorePuzzles(): void {
    ...
    const threeDoorsPuzzle: PuzzleData = {
      ...
      type: 'logic',
      difficulty: 'hard',
      description: 'Three doors stand before you: red, blue, and green. Each is guarded by a sentinel. One guard always tells the truth, one always lies, and one answers randomly. Only one door leads to your destination.',
      instructions: 'You may ask only ONE question to only ONE guard. Then you must choose a door.',
...
// Variable declaration
  ...
  private registerMazePuzzles(): void {
    ...
    const echoNavigationPuzzle: PuzzleData = {
      ...
      type: 'navigation',
      difficulty: 'moderate',
      description: 'The chamber echoes with different sounds from each direction. Follow the correct sequence of echoes to find the hidden passage.',
      instructions: 'Listen to the echoes and select the path that leads to the familiar sound.',
      components: [
        {
          type: 'multiple_choice',
          id: 'echo_sequence',
    ...
    const mirrorPatternPuzzle: PuzzleData = {
      ...
      type: 'pattern',
      difficulty: 'moderate',
      description: 'The mirrors show distorted reflections. Mimic the correct pattern to unlock the hidden passage.',
      instructions: 'Observe the pattern in the distorted reflection and recreate it on the grid.',
      components: [
        {
          type: 'pattern_grid',
          id: 'mirror_pattern',
  ...
  private registerLogicPuzzles(): void {
    ...
    const runeSequencePuzzle: PuzzleData = {
      ...
      type: 'sequence',
      difficulty: 'hard',
      description: 'Ancient runes cover the wall, telling the story of creation. Arrange them in the correct order to unlock their power.',
      instructions: 'Place the runes in the order that tells the complete story from beginning to end.',
      components: [
        {
          type: 'sequence_selector',
          id: 'rune_order',
          label: 'Arrange the runes in order:',
          options: ['Origin Rune', 'Element Rune', 'Life Rune', 'Balance Rune', 'Ending Rune'],
          required: true
        }
      ],
      hints: [
  ...
  private registerNavigationPuzzles(): void {
    ...
    const crossingNavigationPuzzle: PuzzleData = {
      ...
      type: 'navigation',
      difficulty: 'expert',
      description: 'The infinite space around you shifts constantly. Use the dimensional compass to find the stable pathway.',
      instructions: 'Identify the sequence of directions that leads to dimensional stability.',
