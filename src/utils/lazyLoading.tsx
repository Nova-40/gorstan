...
}> = ({ children, fallback = <LoadingFallback /> }) => <Suspense fallback={fallback}>{children}</Suspense>;
...
> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        this.props.fallback || (
          <div className="p-4 text-red-600">Failed to load component. Please refresh the page.</div>
        )
      );
    }
