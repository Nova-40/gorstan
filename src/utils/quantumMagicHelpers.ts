...
export function createNewProgression(playerId: string, preferredElement: QuantumElement = 'void'): QuantumProgression {
  return {
    playerId,
    totalExperience: 0,
    quantumLevel: 0,
...
export function calculateDiscoveryBonus(progression: QuantumProgression, routeId: string): number {
  ...
  const elementArtifacts = Array.from(progression.artifacts.values())
    .filter(artifact => artifact.element === progression.preferredElement);
...
export function getLevelProgress(totalExperience: number, currentLevel: number): {
