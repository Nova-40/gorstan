...
export const ValidationRules = {
  required: <T>(fieldName: string): ValidationRule<T> => ({
    ...
    validate: (value: T) => ({
      ...
      errors:
        value === null || value === undefined || value === '' ? [`${fieldName} is required`] : [],
      warnings: [],
    }),
  }),
  ...
  uniqueArray: <T>(fieldName: string, keyFn?: (item: T) => string): ValidationRule<T[]> => ({
    ...
    validate: (value: T[]) => {
      ...
      return {
        ...
        errors:
          duplicates.length > 0
            ? [`${fieldName} contains duplicates: ${duplicates.join(', ')}`]
            : [],
        warnings: [],
      };
    },
...
export const IdValidation = {
  ...
  getRoomIdRule: (fieldName = 'roomId'): ValidationRule<string> => ({
    ...
    validate: (value: string) => ({
      ...
      errors: IdValidation.isValidRoomId(value)
        ? []
        : [`${fieldName} must be lowercase with underscores only`],
      warnings: [],
    }),
  }),
};
...
export function validateObjectSchema<T extends Record<string, unknown>>(
  ...
  for (const [field, rules] of Object.entries(schema) as Array<
    [keyof T, ValidationRule<unknown>[]]
  >) {
