...
export function handlePollyStalkerLogic(gameState: GameState) {
  ...
  if (!flags.dominicIsDead || flags.killedByPolly) {
    return;
  }
...
export function triggerPollyMercyTakeover(): void {
  appendToConsole(
    'Polly takes the remote control from your hands. She stares at you... then vanishes.',
  );
  appendToConsole('Moments later, two familiar figures shimmer into view.');
  appendToConsole("AL: 'You’ve made a mistake.'");
  appendToConsole(
    "MORTHOS: 'A very big one. If we don’t reboot the multiverse in 4 minutes and 20 seconds, Polly will overwrite reality.'",
  );
