...
function checkAskTwiceRule(
  ...
  if (persona.id !== 'ayla') {
    return null;
  } // Only Ayla implements this rule
...
function getResponseTemplate(
  ...
  return (
    templates[intent] || {
      ...
      variants: ['Could you rephrase that?', "I'm not following.", 'What do you mean?'],
      style_modifiers: {},
    }
  );
}
...
function getTemplatesForNPC(npcId: string): Record<string, ResponseTemplate> {
  ...
  const commonTemplates: Record<string, ResponseTemplate> = {
    ...
    help: {
      ...
      variants: ['What can I do for you?', 'How can I assist?', "Let's figure this out together."],
      style_modifiers: {
...
function hedge(text: string): string {
  const hedges = ['I might be wrong, but ', 'I think ', 'Perhaps ', 'It seems like ', 'I believe '];
...
function addSelfCorrection(text: string): string {
  const corrections = ['Wait—actually, ', 'Let me rephrase: ', 'I mean, ', 'Or rather, '];
