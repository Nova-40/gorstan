...
export function stylize(text: string, voice: Voice): string {
  ...
  if (voice.terseness === 2) {
    ...
    styled = styled.replace(/I think that/g, "I think")
...
function getGenericResponse(npcId: string, topic?: string): string {
  const genericsByNPC: Record<string, Record<string, string[]>> = {
    'morthos': {
      'banter': ["*mechanical whirring*", "Interesting observation.", "Systems nominal."],
      'hint': ["Perhaps we should guide them.", "The solution involves mechanical precision."],
      'lore': ["The old systems were different.", "Before the resets, things were simpler."]
    },
    'al': {
