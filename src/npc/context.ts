...
function determineRoomMood(room: any, gameState: any): string {
  ...
  // Specific room moods
  if (roomId.includes('reset')) {
    return 'urgent';
  }
  if (roomId.includes('cafe') || roomId.includes('kitchen')) {
    return 'cozy';
  }
  if (roomId.includes('library')) {
    return 'quiet';
  }
  if (roomId.includes('maze')) {
    return 'confusing';
  }
  ...
  if (gameState.flags?.dominicKilled) {
    return 'somber';
  }
...
function determineLightLevel(zone: string, room: any): string {
  ...
  if (room?.id?.includes('hidden')) {
    return 'dim';
  }
  if (room?.id?.includes('lab')) {
    return 'bright';
  }
  return 'normal';
}
...
function extractActiveQuests(gameState: any): string[] {
  ...
  if (gameState.flags?.seekingResetRoom) {
    quests.push('find_reset_room');
  }
  if (gameState.flags?.pollyTakeoverActive) {
    quests.push('stop_polly');
  }
  if (gameState.flags?.hasSchrodingerCoin && !gameState.flags?.coinUsed) {
    quests.push('use_coin');
  }
  if (gameState.flags?.needsLibraryAccess) {
    quests.push('access_library');
  }
  if (gameState.flags?.lookingForDominic) {
    quests.push('find_dominic');
  }
...
export function analyzePlayerBehavior(context: ContextSnapshot): {
  ...
  const isExploring =
    context.availableExits.length > 1 &&
  ...
  const isRushing =
    context.sessionInfo.playTime > 0 &&
  ...
  if (isExploring && !isStuck) {
    confidenceLevel += 0.3;
  }
  if (isStuck) {
    confidenceLevel -= 0.4;
  }
  if (isRushing && !isStuck) {
    confidenceLevel += 0.2;
  }
  if (needsGuidance) {
    confidenceLevel -= 0.2;
...
export function getLocationHints(context: ContextSnapshot): string[] {
  ...
  if (context.roomId.includes('reset')) {
    hints.push('critical_choice', 'point_of_no_return');
  }
  if (context.roomId.includes('library')) {
    hints.push('research_needed', 'information_available');
  }
  if (context.roomId.includes('cafe')) {
    hints.push('social_space', 'rest_area');
  }
  if (context.roomId.includes('maze')) {
    hints.push('navigation_challenge', 'patience_required');
  }

  // Item-based hints
  if (context.inventory.includes('schrodingerCoin')) {
    hints.push('quantum_paradox', 'choice_matters');
  }
  if (context.inventory.includes('napkin')) {
    hints.push('research_clue', 'library_relevant');
  }
