...
function shouldAddHedging(persona: any, memory: NPCMemoryState, confidence: number): boolean {
  ...
  if (persona.tone.caution > 0.6) {
    return Math.random() < 0.4;
  }
  if (confidence < 0.7) {
    return Math.random() < 0.3;
  }
  if (memory.relationshipLevel < 0.2) {
...
function shouldAddCorrection(persona: any, memory: NPCMemoryState): boolean {
  ...
  if (memory.relationshipLevel > 0.6) {
...
function addSelfCorrection(response: string, persona: any): string {
  const corrections = ['Actually, ', 'Well, ', 'I mean, ', 'Or rather, ', 'Wait—'];
...
function addMemoryHook(response: string, memory: NPCMemoryState, persona: any): string {
  ...
  const playerMentioned = recentConversation.find(
    (turn) =>
      turn.speaker === 'player' && (turn.message.includes('you') || turn.message.includes('said')),
  );
 ...
 */
function adjustEmotionalTemperature(
  response: string,
