...
export class GroupChatManager {
  ...
  static triggerWendellGreeting(context: GroupChatContext): void {
    const { state, dispatch, roomId } = context;
    const hasWendell = context.npcsInRoom.some(
      (npc) => npc.id === 'mr_wendell' || npc.id === 'wendell' || npc.id === 'mrwendell',
    );

    if (hasWendell) {
      const ctx: ConversationContext = { state, dispatch, roomId };
      const otherNpcs = context.npcsInRoom.filter(
        (npc) => npc.id !== 'mr_wendell' && npc.id !== 'wendell' && npc.id !== 'mrwendell',
      );
  ...
  static triggerAlliancePitch(context: GroupChatContext): void {
    ...
    if (state.flags?.alliancePitchStarted) {
      return;
    }
  ...
  static processAllianceChoice(choice: string, context: GroupChatContext): boolean {
    ...
    if (!state.flags?.awaitingAllianceChoice) {
      return false;
    }
    ...
    if (lowerChoice.includes('ally morthos') || lowerChoice.includes('choose morthos')) {
      ...
      lowerChoice.includes('ally neither') ||
      lowerChoice.includes('stay neutral') ||
      lowerChoice.includes('no alliance')
    ) {
  ...
  static triggerNPCDiscussion(context: GroupChatContext, playerMessage?: string): void {
    ...
    if (playerMessage && state.flags?.npcDiscussionActive) {
      const speaker =
        context.npcsInRoom[Math.floor(Math.random() * context.npcsInRoom.length)] ?? ({ id: 'al' } as NPC);
      ...
      const discussionTopics = [
        {
          starter: 'al',
          message:
            '*checks documentation* We need to discuss the anomalous readings from the lattice sector.',
          follow: 'morthos',
          response:
            '*nods seriously* The dimensional fluctuations are increasing. Player, please give us a moment.',
        },
        {
          starter: 'ayla',
          message:
            '*observes reality threads* The convergence patterns are shifting. This requires careful consideration.',
          follow: 'al',
          response:
            '*immediately alert* Convergence shift? *to player* Please hold all questions - this is critical.',
        },
        {
          starter: 'morthos',
          message:
            "*shadows coil with concern* Something's wrong with the underlying infrastructure.",
          follow: 'ayla',
          response:
            '*threads of reality shimmer* Yes, I sense it too. *to player* We need to focus on this briefly.',
        },
      ];

      const topic = discussionTopics.length > 0 ? discussionTopics[Math.floor(Math.random() * discussionTopics.length)] : undefined;

      if (!topic) {
        // No topic available — nothing to schedule
        return;
      }
