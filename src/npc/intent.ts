...
> = {
  ...
  help: {
    keywords: ['help', 'assist', 'stuck', 'lost', 'confused', 'guidance', 'hint', 'clue'],
    negative_keywords: [],
    confidence_boost: 0.9,
  },
  location: {
    keywords: ['where', 'location', 'room', 'place', 'zone', 'area', 'go to', 'find', 'reach'],
    negative_keywords: ['reset location', 'save location'], // Avoid confusion with save mechanics
    confidence_boost: 0.7,
  },
  inventory: {
    keywords: ['inventory', 'items', 'carrying', 'have', 'possess', 'belongings'],
    negative_keywords: [],
    confidence_boost: 0.8,
  },
  puzzle_hint: {
    keywords: ['puzzle', 'solution', 'solve', 'answer', 'how do i', 'what should i do'],
    negative_keywords: [],
    confidence_boost: 0.6,
  },
  lore: {
    keywords: ['story', 'history', 'background', 'lore', 'why', 'explain', 'tell me about'],
    negative_keywords: [],
    confidence_boost: 0.5,
  },
  ethics: {
    keywords: ['should i', 'is it wrong', 'ethical', 'moral', 'right thing', 'consequences'],
    negative_keywords: [],
    confidence_boost: 0.7,
  },
  insult: {
  ...
  meta: {
    keywords: ['game', 'developer', 'fourth wall', 'real', 'ai', 'program', 'code'],
    negative_keywords: [],
    confidence_boost: 0.6,
  },
  time_pressure: {
    keywords: ['timer', 'time', 'hurry', 'quick', 'fast', 'urgent', 'polly', 'takeover'],
    negative_keywords: [],
    confidence_boost: 0.8,
 ...
 */
export function classifyIntent(utterance: string, context: any = {}): IntentResult {
  ...
  for (const [intent, pattern] of Object.entries(INTENT_PATTERNS)) {
    ...
    if (positiveMatches > 0) {
      confidence = (positiveMatches / pattern.keywords.length) * pattern.confidence_boost;
...
function getContextClues(utterance: string, context: any): string[] {
  ...
  // Location context
  if (context.roomId) {
    if (context.roomId.includes('reset')) {
      clues.push('in_critical_room');
    }
    if (context.roomId.includes('library')) {
      clues.push('in_knowledge_area');
    }
    if (context.roomId.includes('maze')) {
      clues.push('in_navigation_challenge');
    }
  }
  ...
  if (utterance.split(' ').length > 10) {
    clues.push('detailed_query');
  }
  if (utterance.split(' ').length < 3) {
    clues.push('brief_query');
  }

  // Question types
  if (utterance.startsWith('what')) {
    clues.push('what_question');
  }
  if (utterance.startsWith('where')) {
    clues.push('where_question');
  }
  if (utterance.startsWith('how')) {
    clues.push('how_question');
  }
  if (utterance.startsWith('why')) {
    clues.push('why_question');
  }
  if (utterance.startsWith('should')) {
    clues.push('decision_question');
  }
...
export function isPuzzleSolutionRequest(
  ...
  if (entities.includes('coin') || normalizedUtterance.includes('schrodinger')) {
    specificPuzzle = 'schrodinger_coin';
  } else if (entities.includes('blue switch') || entities.includes('blue button')) {
  ...
  let urgencyLevel = 0;
  if (normalizedUtterance.includes('urgent') || normalizedUtterance.includes('hurry')) {
    urgencyLevel += 0.5;
  }
  if (normalizedUtterance.includes('please') || normalizedUtterance.includes('help')) {
    urgencyLevel += 0.3;
  }
  if (normalizedUtterance.includes('stuck') || normalizedUtterance.includes('lost')) {
    urgencyLevel += 0.4;
  }
...
export function disambiguateIntent(results: IntentResult[], context: any): IntentResult {
  ...
  if (results.length > 1) {
    ...
    if (context.timers?.pollyTakeover?.active) {
      const timePressureResult = sorted.find((r) => r.intent === 'time_pressure');
    ...
    if (context.roomId?.includes('maze') || context.roomId?.includes('puzzle')) {
