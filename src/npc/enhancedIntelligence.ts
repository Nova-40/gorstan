...
class NPCIntelligenceEngine {
  ...
  generateResponse(npcId: string, npc: NPC, playerInput: string, state: LocalGameState): string {
  ...
  private analyzeSentiment(input: string): number {
    ...
    words.forEach((word) => {
      if (positive.some((p) => word.includes(p))) {
        score += 0.1;
      }
      if (negative.some((n) => word.includes(n))) {
        score -= 0.1;
      }
    });
  ...
  private analyzeIntent(input: string): string {
    ...
    if (lower.includes('where') || lower.includes('how')) {
      return 'guidance';
    }
    if (lower.includes('who') || lower.includes('what')) {
      return 'information';
    }
    if (lower.includes('thank') || lower.includes('goodbye')) {
      return 'social';
    }
    if (lower.includes('?')) {
      return 'question';
  ...
  private updateRelationship(
    ...
    if (memory.relationship > 0.5) {
      memory.conversationStyle = 'friendly';
    } else if (memory.relationship < -0.5) {
      memory.conversationStyle = 'hostile';
    } else if (memory.relationship < -0.2) {
  ...
  generateProactiveConversation(npcId: string, npc: NPC, state: LocalGameState): string {
    ...
    const archetype =
      Object.keys(NPC_INTELLIGENCE_PROFILES).find(
        (key) =>
          NPC_INTELLIGENCE_PROFILES[key as keyof typeof NPC_INTELLIGENCE_PROFILES] === profile,
      ) || 'guide';

    return (
      proactiveMessages[archetype as keyof typeof proactiveMessages] ||
