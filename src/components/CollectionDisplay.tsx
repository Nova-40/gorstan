...
export const CollectionDisplay: React.FC<CollectionDisplayProps> = ({
  ...
  const getItemType = (itemName: string): ItemData['type'] => {
    if (['coffee', 'healing_potion'].includes(itemName)) {
      return 'consumable';
    }
    if (['iron_key'].includes(itemName)) {
      return 'key';
    }
    if (['ancient_scroll'].includes(itemName)) {
      return 'lore';
    }
    if (['crystal_shard', 'rune_stone'].includes(itemName)) {
      return 'material';
    }
    return 'artifact';
  };
  ...
  const getItemRarity = (itemName: string): ItemData['rarity'] => {
    if (['mystical_orb'].includes(itemName)) {
      return 'legendary';
    }
    if (['crystal_shard', 'ancient_scroll'].includes(itemName)) {
      return 'epic';
    }
    if (['rune_stone', 'golden_coin'].includes(itemName)) {
      return 'rare';
    }
    if (['iron_key'].includes(itemName)) {
      return 'uncommon';
  ...
  const getItemLore = (itemName: string): string => {
    const lore: Record<string, string> = {
      ancient_scroll:
        'Found in the ruins of the Great Library, this scroll contains the lost art of dimensional navigation.',
      crystal_shard:
        'Legend speaks of nine crystal shards scattered across the realms, each containing a fragment of pure creation.',
      mystical_orb:
        'The Orb of Eternities, said to show glimpses of all possible futures to those brave enough to gaze within.',
      rune_stone:
        'These stones were carved by the First Mages to channel and focus magical energies.',
      golden_coin:
        'Minted in the Golden Age of Gorstan, these coins were blessed by the ancient prosperity spirits.',
    };
                    ...
                    {filteredInventory.map((item) => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div
                            className={`p-2 rounded ${getRarityColor(item.rarity).replace('text-', 'bg-').replace('border-', '').replace('-400', '-900/50').replace('-500', '-900/50')}`}
                          >
                          ...
                          <div className="flex-1 min-w-0">
                            <h3
                              className={`font-bold ${getRarityColor(item.rarity).split(' ')[0]}`}
                            >
                              {item.name}
                            </h3>
