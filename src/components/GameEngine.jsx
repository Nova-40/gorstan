...
const GameEngine = ({ playerName, introChoice = "jump", onError }) => {
  ...
  useEffect(() => {
    engineRef.current = {
      ...
      say: (text) => dispatch({ type: "LOG", payload: `You say: "${text}"` }),
      log: (text) => dispatch({ type: "LOG", payload: text }),
      getState: () => state,
      dispatch,
    };
  }, []);
  ...
  if (!currentRoom) {
    ...
    return <div className="text-red-400 bg-black p-6" role="alert">⚠️ Room "{state.currentRoom}" not found.</div>;
  }
  ...
  return (
    <div className="p-4 text-green-200 font-mono bg-black min-h-screen">
      ...
      <div className="mt-4 flex flex-col gap-3 items-center">
        <CommandInput command={command} setCommand={setCommand} onSubmit={handleCommandSubmit} />
