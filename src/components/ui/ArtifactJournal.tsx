...
const ArtifactJournal: React.FC<ArtifactJournalProps> = ({
  ...
  const getFilteredLore = () => {
    ...
    return lore.sort((a: ArtifactLoreEntry, b: ArtifactLoreEntry) => {
      switch (sortBy) {
        ...
        case 'importance':
          return (
            (b.metadata.significance === 'critical'
              ? 4
              : b.metadata.significance === 'high'
                ? 3
                : b.metadata.significance === 'medium'
                  ? 2
                  : 1) -
            (a.metadata.significance === 'critical'
              ? 4
              : a.metadata.significance === 'high'
                ? 3
                : a.metadata.significance === 'medium'
                  ? 2
                  : 1)
          );
        case 'date':
  ...
  const getTabCount = (tab: TabType) => {
    switch (tab) {
      case 'lore':
        return getFilteredLore().length;
      case 'visions':
        return getFilteredVisions().length;
      case 'bonds':
        return bonds.length;
      case 'communications':
        return getFilteredCommunications().length;
      case 'research':
        return Object.keys(archive.personalNotes).length;
      default:
        return 0;
    }
  };
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-300 mb-2">
                        Arc: <span className="capitalize">{lore.arc}</span> • Style:{' '}
                        <span className="capitalize">{lore.style}</span>
                      </div>
                      <div className="text-xs text-gray-400">
                        Significance:{' '}
                        <span className="capitalize">{lore.metadata.significance}</span>
                      </div>
                    </div>
                        ...
                        className={`text-xs px-2 py-1 rounded capitalize ${
                          vision.visionType === 'memory'
                            ...
                            : vision.visionType === 'prophecy'
                              ...
                              : vision.visionType === 'warning'
                                ? 'bg-red-900 text-red-200'
                                : vision.visionType === 'guidance'
          ...
          <div className="space-y-4">
            {bonds.map((bond) => {
              const communications = getFilteredCommunications()
                .filter((comm) => comm.artifactId === bond.artifactId)
                .slice(0, 5);
              ...
              return (
                <ArtifactBondDisplay
                  ...
                  {...(onAddNote ? { onAddNote: (note: string) => onAddNote(bond.artifactId, note) } : {})}
                />
              );
            })}
          </div>
  ...
  return (
    <div
      className={`fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center p-4 z-40 ${className}`}
    >
      <div className="max-w-6xl w-full h-full max-h-[90vh] bg-gray-800 border border-gray-600 rounded-lg overflow-hidden flex flex-col">
        ...
        <div className="p-4 border-b border-gray-600 space-y-4">
          ...
          <div className="flex space-x-1">
            {(['lore', 'visions', 'bonds', 'communications', 'research'] as TabType[]).map(
              (tab) => (
                <button
                  key={tab}
                ...
                >
                  ...
                  <span className="ml-2 text-sm bg-gray-600 px-1 rounded">{getTabCount(tab)}</span>
                </button>
              ),
            )}
          </div>
