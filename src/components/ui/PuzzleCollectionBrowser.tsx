...
const PuzzleCollectionBrowser: React.FC<PuzzleCollectionBrowserProps> = ({
  ...
  const getFilteredPuzzles = () => {
    ...
    return filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'completion':
          return (b.analytics.completions / Math.max(1, b.analytics.attempts)) - 
  ...
  const getRecommendedPuzzles = () => {
    ...
    return puzzles.filter(p => {
      ...
      return playerDifficulty.recommendations.suggestedTypes.includes(p.type) &&
  ...
  const getDifficultyColor = (difficulty: PuzzleDifficulty) => {
    switch (difficulty) {
      case 'trivial': return 'text-green-400 bg-green-900';
      case 'easy': return 'text-blue-400 bg-blue-900';
      case 'medium': return 'text-yellow-400 bg-yellow-900';
      case 'hard': return 'text-orange-400 bg-orange-900';
      case 'expert': return 'text-red-400 bg-red-900';
      case 'quantum': return 'text-purple-400 bg-purple-900';
              ...
              <div className="flex-1">
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  <span className={`px-2 py-1 text-xs rounded capitalize ${
                    collection.theme === 'tutorial' ? 'bg-green-900 text-green-200' :
                    collection.theme === 'quantum' ? 'bg-purple-900 text-purple-200' :
                    collection.theme === 'master' ? 'bg-red-900 text-red-200' :
  ...
  const renderPuzzleList = (puzzleList: LogicPuzzle[]) => (
    <div className="space-y-3">
      {puzzleList.map(puzzle => {
        const completionRate = puzzle.analytics.attempts > 0 ? 
          (puzzle.analytics.completions / puzzle.analytics.attempts) * 100 : 0;
              ...
              <div className="flex items-start space-x-3 flex-1">
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className={`px-2 py-1 text-xs rounded capitalize ${getDifficultyColor(puzzle.difficulty)}`}>
                      {puzzle.difficulty}
                    </span>
        ...
        <div className="bg-gray-700 rounded-lg p-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="ml-2 text-white">{(playerDifficulty.confidence * 100).toFixed(0)}%</span>
            </div>
            <div>
              <span className="text-gray-400">Recommended:</span>
              <span className="ml-2 text-yellow-400 capitalize">{playerDifficulty.recommendations.nextDifficulty}</span>
            </div>
            <div>
            ...
            <div className="mt-4">
              ...
              <div className="space-y-1">
                {playerDifficulty.recommendations.skillGaps.map((gap, index) => (
                  <div key={index} className="text-xs text-orange-400">• {gap}</div>
                ))}
              </div>
            </div>
          )}
