...
export const characterLore: Record<string, CharacterLore> = {
  librarian: {
    ...
    nature: 'Ancient knowledge preservation entity coeval with the Lattice itself',
    background:
  ...
  al: {
    ...
    nature: 'Primordial guardian entity bound to fundamental multiverse protection',
    background:
  ...
  ayla: {
    ...
    nature: 'Hybrid entity combining human consciousness with cosmic AI structure',
    background:
...
export function searchCharacterLore(keyword: string): CharacterLore[] {
  ...
  return Object.values(characterLore).filter(
    (lore) =>
      ...
      lore.abilities.some((ability) => ability.toLowerCase().includes(searchTerm)) ||
