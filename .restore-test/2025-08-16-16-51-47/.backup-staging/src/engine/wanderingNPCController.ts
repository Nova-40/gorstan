/*
  Gorstan – Copyright © 2025 Geoff W    for (const [_npcId, npc] of npcRegistry.entries()) {bster. All Rights Reserved.
...
export function handleRoomEntryForWanderingNPCs(
  ...
  for (const [npcId, npc] of npcRegistry.entries()) {
    ...
    const isVisible =
      typeof npc.shouldBeVisible === 'function' ? npc.shouldBeVisible(state, room) : true;
...
export function initializeWanderingNPCs(_state: LocalGameState, dispatch: Dispatch<GameAction>) {
  for (const [_npcId, npc] of npcRegistry.entries()) {
