...
export class DynamicContentGenerator {
  ...
  private ruleUsage: Map<string, { count: number; lastUsed: number }> = new Map();
  ...
  private initializeDefaultRules(): void {
    ...
    this.addRule({
      ...
      description: 'Generate story elements based on player exploration patterns',
    ...
    this.addRule({
      ...
      description: 'Provide additional details when players examine rooms thoroughly',
  ...
  private findApplicableRules(
    ...
    for (const rule of this.rules.values()) {
      if (this.isRuleApplicable(rule, gameState, currentRoom, recentCommands, timeInRoom)) {
...
Content:`;
   ...
   */
  private storeGeneratedContent(roomId: string, content: GeneratedContent): void {
   ...
   */
  public getRecentContent(roomId: string, limit: number = 5): GeneratedContent[] {
    const roomContent = this.generatedContent.get(roomId) || [];
  ...
  public resetSession(): void {
    for (const [_ruleId, usage] of this.ruleUsage.entries()) {
      usage.count = 0;
    }
  }
