...
export function evaluateWendellSpawn(
  ...
  // Variable declaration
  const isSafeRoom = safeRooms.some((safe) => room.id.toLowerCase().includes(safe.toLowerCase()));
...
function checkForCursedItems(gameState: LocalGameState): boolean {
  ...
  return gameState.player.inventory.some((item: any) => cursedItems.includes(item.toLowerCase()));
}
