...
export const useFlags = () => {
  ...
  const setFlag = useCallback(
    (key: FlagKey, value: boolean | string | number) => {
  ...
  const getFlag = useCallback(
    <T = boolean | string | number>(key: FlagKey, defaultValue: T): T => {
      const flagKey = String(key); // Ensure key is a string
  ...
  const hasAnyFlag = useCallback(
    (keys: Array<FlagKey>): boolean => {
      return keys.some((key) => hasFlag(key));
    },
    [hasFlag],
  );

  const hasAllFlags = useCallback(
    (keys: Array<FlagKey>): boolean => {
  ...
  const incrementFlag = useCallback(
    (key: FlagKey, amount: number = 1) => {
  ...
  const getFlagsMatching = useCallback(
    (pattern: RegExp): Record<string, any> => {
