...
import { safeObjectIteration } from './index';
...
export function validateGlobalItemManagement(
  ...
  // Use new safe object iteration utility
  safeObjectIteration(
    roomMap,
    (roomId, room) => {
      ...
      if (room.items && Array.isArray(room.items)) {
        ...
        itemIds.forEach((itemId) => {
          ...
          if (!itemData) {
            ...
            // Check spawn room configuration
            ...
            // Check exclude room configuration
    ...
    (error, roomId) => {
      // Error handler for safe iteration
      result.errors.push(`Error processing room ${String(roomId)}: ${error.message}`);
      result.isValid = false;
    },
  );

  // Validate individual ITEMS configuration
  ITEMS.forEach((item) => {
    ...
    // Special validations for specific items
    if (item.id === 'dominic') {
