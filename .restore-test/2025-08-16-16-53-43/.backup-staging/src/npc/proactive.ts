 ...
 */
function evaluateAylaPrompts(
  context: ContextSnapshot,
  memory: NPCMemoryState,
): ProactivePrompt | null {
  // Flash when player seems stuck (checking session info for stuck indicators)
  if (context.sessionInfo.roomVisitCount > 5 && context.sessionInfo.idleTime > 30000) {
    return {
      npcId: 'ayla',
      type: 'visual_flash',
      ...
      accessibility: {
        screen_reader_text:
          "Ayla seems to have noticed you're having trouble and might have suggestions",
 ...
 */
function evaluatePollyUrgency(
  context: ContextSnapshot,
  _memory: NPCMemoryState,
): ProactivePrompt | null {
  ...
  if (timeRemaining < 30000) {
    return {
      ...
      accessibility: {
        screen_reader_text:
          "ALERT: Polly's system takeover is imminent - less than 30 seconds remaining",
 ...
 */
function evaluateWendellLibraryPrompts(
  context: ContextSnapshot,
  memory: NPCMemoryState,
): ProactivePrompt | null {
  ...
  if (context.recentConsoleEvents.includes('book_examined') && memory.totalInteractions < 3) {
    return {
      ...
      accessibility: {
        screen_reader_text:
          'Wendell has noticed your choice of book and seems to have thoughts about it',
      },
    };
  }
 ...
 * Dominic's system observations
 */
function evaluateDominicPrompts(
  context: ContextSnapshot,
  _memory: NPCMemoryState,
): ProactivePrompt | null {
  ...
  if (context.recentConsoleEvents.includes('glitch_detected')) {
    return {
      ...
      accessibility: {
        screen_reader_text:
          'Dominic has detected system irregularities and might have technical insights',
      },
    };
  }
