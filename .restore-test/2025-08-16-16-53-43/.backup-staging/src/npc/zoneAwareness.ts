...
export class ZoneAwarenessProvider {
  ...
  getConnectedZones(zoneId: string): string[] {
    ...
    for (const [_roomId, mapping] of this.roomZoneMappings.entries()) {
  ...
  getZoneStats(): {
    ...
    const totalTransitionRules = Array.from(this.transitionRules.values()).reduce(
  ...
  private checkCrossZoneMovement(
    ...
    const applicableRule = rules.find(
      (rule) => rule.toZone === toZone && rule.allowedNPCTypes.includes(npcType),
    );
  ...
  private findAlternativeZones(npcType: string, excludeZone: string): string[] {
    ...
    for (const [zoneId, _zone] of this.zones.entries()) {
