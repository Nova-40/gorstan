...
export function attemptPickup(
  ...
  if (state.inventory.includes(item)) {
    dispatchMessage({
      type: 'ADD_MESSAGE',
      payload: { text: `You already have the ${item}.`, type: 'error' },
    });
    return;
  }
    ...
    if (hasRunbag && state.inventory.length >= 12) {
      if (Math.random() < 0.05) {
        ...
        dispatchMessage({
          type: 'ADD_MESSAGE',
          payload: {
            text: `The runbag bursts spectacularly. Your belongings scatter into the void.`,
            type: 'error',
          },
        });
      } else {
        dispatchMessage({
          type: 'ADD_MESSAGE',
          payload: { text: `There is no more room in your bag.`, type: 'error' },
        });
      }
    } else {
      dispatch({ type: 'SET_INVENTORY', payload: [] });
      dispatchMessage({
        type: 'ADD_MESSAGE',
        payload: {
          text: `Your pockets rip under the strain. Everything falls to the floor.`,
          type: 'error',
        },
      });
    }
  ...
  dispatchMessage({
    type: 'ADD_MESSAGE',
    payload: { text: `You pick up the ${item}.`, type: 'success' },
  });
}
