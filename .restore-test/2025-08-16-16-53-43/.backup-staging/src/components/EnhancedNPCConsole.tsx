...
const EnhancedNPCConsole: React.FC<EnhancedNPCConsoleProps> = ({
  ...
  const getSpeakerBadgeColor = (npcId: string, _context?: string): string => {
  ...
  const getSpeakerDisplay = (message: EnhancedDialogueMessage): React.ReactElement => {
    ...
    return (
      <div className="speaker-info npc">
        ...
        />
        <span className="speaker-name" style={{ color: badgeColor }} data-npc={message.npcId}>
  ...
  const getSingleGreeting = (_npcId: string): string => {
  ...
  const getConversationContext = (npcId: string): 'competitive' | 'cooperative' | 'neutral' => {
    ...
    if (
      (npcId === 'al' || npcId === 'morthos') &&
      npcs.some((n) => n.id === 'al') &&
      npcs.some((n) => n.id === 'morthos')
    ) {
      return 'competitive';
    }
  ...
  const handleSendMessage = () => {
    ...
    if (
      lowerMessage.includes('i choose you morthos') ||
      lowerMessage.includes('i chose you morthos')
    ) {
      dispatch({ type: 'SET_ALLIANCE', payload: { ally: 'morthos', points: 20 } });
    } else if (
      lowerMessage.includes('i choose you al') ||
      lowerMessage.includes('i chose you al')
    ) {
      dispatch({ type: 'SET_ALLIANCE', payload: { ally: 'al', points: 40 } });
    } else if (
      lowerMessage.includes('i choose both') ||
      lowerMessage.includes('i chose both') ||
      lowerMessage.includes('i choose you both') ||
      lowerMessage.includes('i chose you both')
    ) {
    ...
    const isRudeToWendell = (message: string, npcs: typeof realNpcs): boolean => {
      const hasWendell = npcs.some(
        (npc) => npc.id === 'mr_wendell' || npc.name.toLowerCase().includes('wendell'),
      );
    ...
    if (isGroupConversation) {
      ...
      console.log(
        `[Enhanced NPC Console] Individual chat - ${primaryNpc?.name} (${primaryNpc?.id}) will respond`,
      );
    }
    ...
    typingTimeoutRef.current = setTimeout(() => {
      console.log(
        `[Enhanced NPC Console] 🚨 GUARANTEED typing timeout triggered - force clearing typing state`,
      );
      setIsTyping(false);
      setTypingNpcId(null);
  ...
  const determineRespondingNpc = (message: string): string => {
    ...
    if (
      ...
      const availableNpcs = realNpcs
        .map((npc) => npc.id)
        .filter((id) => npcWeights[id] !== undefined);
  ...
  const handleGroupResponse = async (playerMessage: string) => {
    ...
    console.log(
      `[Enhanced NPC Console] Group response - ${respondingNpc?.name} (${respondingNpcId}) responding to: "${playerMessage}"`,
    );
    console.log(
      `[Enhanced NPC Console] Available NPCs:`,
      realNpcs.map((n) => `${n.name}(${n.id})`),
    );

    if (respondingNpc) {
      try {
        console.log(
          `[Enhanced NPC Console] 🎬 Calling generateContextualResponse for ${respondingNpc.name}`,
        );
        const response = await generateContextualResponse(respondingNpcId, playerMessage);
        console.log(
          `[Enhanced NPC Console] 📝 Generated response from ${respondingNpc.name}:`,
          response,
        );

        if (!response || response.trim().length === 0) {
          console.error(
            `[Enhanced NPC Console] ❌ Empty response from generateContextualResponse for ${respondingNpc.name}`,
          );
          ...
          console.log(
            `[Enhanced NPC Console] 🚨 FORCING response for ${respondingNpc.name}: ${forceResponse}`,
          );
        ...
        console.log(
          `[Enhanced NPC Console] ✅ Valid response from ${respondingNpc.name}: ${response}`,
        );
        ...
        console.log(
          `[Enhanced NPC Console] ✅ Typing state cleared for successful group response from ${respondingNpc.name}`,
        );
        ...
        if (shouldTriggerInterNPCExchange(respondingNpcId, playerMessage)) {
          setTimeout(
            async () => {
              ...
              if (reactionNpcId) {
                const reactionResponse = await generateInterNPCResponse(
                  reactionNpcId,
                  respondingNpcId,
                  response,
                );
  ...
  const handleSingleResponse = async (playerMessage: string) => {
    console.log(
      `[Enhanced NPC Console] handleSingleResponse called for ${primaryNpc?.name} (${primaryNpc?.id})`,
    );
    try {
      ...
      console.log(
        `[Enhanced NPC Console] ✅ Typing state cleared for successful single response from ${primaryNpc!.name}`,
      );
    } catch (error) {
  ...
  const generateContextualResponse = async (
    ...
    console.log(
      `[Enhanced NPC Console] 🎬 Generating response for ${npcId} to: "${playerMessage}"`,
    );
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => {
          console.log(
            `[Enhanced NPC Console] ⏰ AI timeout for ${npcId} after 5 seconds - falling back to scripted`,
          );
      ...
      console.log(
        `[Enhanced NPC Console] 🔄 AI failed for ${npcId} (${errorMessage}), using scripted fallback`,
      );
    }
  ...
  const generateInterNPCResponse = async (
    reactingNpcId: string,
    originalNpcId: string,
    originalResponse: string,
  ): Promise<string> => {
    // Try AI-enhanced NPC-to-NPC responses first
    try {
      const aiResponse = await generateNPCToNPCResponse(
        reactingNpcId,
        originalNpcId,
        originalResponse,
      );
      ...
      console.warn(
        `AI-powered inter-NPC response failed for ${reactingNpcId} → ${originalNpcId}:`,
        error,
      );
    }
                ...
                <div className="message-header">
                  <div className="speaker-info npc">
                    ...
                    <span className="speaker-name">
                      {typingNpcId ? realNpcs.find((n) => n.id === typingNpcId)?.name : 'Someone'}{' '}
                      is typing...
                    </span>
                  </div>
                </div>
          ...
          <div className="input-area">
            ...
            {isGroupConversation && (
              <div className="conversation-hints">
                <small>
                  💡 Tip: Address NPCs by name (e.g., "Al, what do you think?") or ask topic-based
                  questions
                </small>
              </div>
            )}
