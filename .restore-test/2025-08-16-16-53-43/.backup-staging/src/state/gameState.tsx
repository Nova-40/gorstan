...
import { setGameState } from './stateUtils';
...
export function getGameState(): LocalGameState | null {
  ...
  return state as LocalGameState;
}
...
export function saveGame(slot: number): void {
  // Ensure state is explicitly typed as LocalGameState
  const state = getGameState();
  if (!state) {
    console.warn('Cannot save game: state is null.');
    return;
  }

  const saveFile = {
    version: SaveManager.CURRENT_VERSION,
    playerName: state.playerName || 'Unknown Player',
    progress: state.progress || {},
    timestamp: new Date().toISOString(),
  };
  SaveManager.save(slot, saveFile);
}
...
export function loadGame(slot: number): void {
  ...
  const saveFile = SaveManager.load(slot);
  if (saveFile) {
    ...
    console.warn('Failed to load game. Save file is invalid or missing.');
  }
}

// Use a type guard to ensure state is LocalGameState
function isLocalGameState(state: any): state is LocalGameState {
  return state && typeof state === 'object' && 'progress' in state && 'playerName' in state;
}

const state = getGameState();
if (!isLocalGameState(state)) {
  console.warn('Game state is null or invalid. Cannot proceed.');
}
